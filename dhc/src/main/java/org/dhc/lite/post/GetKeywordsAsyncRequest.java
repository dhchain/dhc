...
public class GetKeywordsAsyncRequest extends Message {
	...
	public GetKeywordsAsyncRequest(DhcAddress myDhcAddress, DhcAddress dhcAddress, String transactionId) {
	...
	public void process(Peer peer) {
		...
		if(DhcAddress.getMyDhcAddress().isFromTheSameShard(dhcAddress, Blockchain.getInstance().getPower())) {
			...
			GetKeywordsAsyncReply message  = new GetKeywordsAsyncReply(myDhcAddress, dhcAddress, keywords);
	...
	public String toString() {
		String str = String.format("GetKeywordsAsyncRequest %s-%s-%s-%s", myDhcAddress, dhcAddress, getCorrelationId());
		return str;
	}
