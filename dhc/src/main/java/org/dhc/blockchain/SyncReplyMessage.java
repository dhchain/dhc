...
public class SyncReplyMessage extends Message {
	...
	public void process(Peer peer) {
		...
		if(!myBlocks(blocks)) {
			...
			synchronizer.incNotify(peer, String.format("Not my blocks blocksKey=%s, peersKey=%s, myKey=%s", blocksKey, peer.getDhcAddress().getBinary(blocksKey.length()), DhcAddress.getMyDhcAddress().getBinary(blocksKey.length())) );
			return;
		}
